...
import GeneNotFound from '../GenePage/GeneNotFound'
...
import HaplotypeGenePage from './HaplotypeGenePage'
...
const HaplotypeGenePageContainer = ({ datasetId, geneIdOrSymbol }: Props) => {
  const variables = {
  ...
  return (
    <BaseQuery operationName={operationName} query={query} variables={variables}>
      {({ data, error, graphQLErrors, loading }: any) => {
        ...
        return <HaplotypeGenePage datasetId={datasetId} gene={gene} geneId={data.gene.gene_id} />
      }}
    </BaseQuery>
  )
}

export default HaplotypeGenePageContainer
