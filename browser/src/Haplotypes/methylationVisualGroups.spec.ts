...
const point = (pos: number, mean = 50, overrides: Partial<MethylationSummaryPoint> = {}): MethylationSummaryPoint => ({
  chrom: 'chr22', pos1: pos, pos2: pos + 1, mean_methylation: mean,
  mean_coverage: 20, num_samples: 100, std_methylation: 5,
  min_methylation: mean - 5, max_methylation: mean + 5, ...overrides,
})
...
describe('buildMethylationVisualGroups', () => {
  test('is deterministic and sorts input without mutating it', () => {
    ...
    expect(buildMethylationVisualGroups(input)[0].sites.map((site) => site.pos1)).toEqual([100, 200, 300])
  })

  test('breaks on chromosome changes and gaps over, but not exactly, 1 kb', () => {
    const groups = buildMethylationVisualGroups([
      point(100), point(1100), point(2101), point(2200, 50, { chrom: 'chrX' }),
    ])
    expect(groups.map((group) => group.siteCount)).toEqual([2, 1, 1])
    expect(groups.map((group) => group.boundaryReason)).toEqual(['display-start', 'gap-over-1kb', 'chromosome-change'])
  })

  test('caps groups at 200 CpGs and preserves an isolated site', () => {
    const groups = buildMethylationVisualGroups(Array.from({ length: 201 }, (_, index) => point(index * 2)))
  ...
  test('finds deterministic synthetic step changes but leaves a flat run together', () => {
    ...
    const stepped = buildMethylationVisualGroups([
      point(1, 20), point(2, 21), point(3, 70), point(4, 71), point(5, 25), point(6, 24),
    ])
    expect(flat).toHaveLength(1)
    expect(stepped.map((group) => group.siteCount)).toEqual([2, 2, 2])
    expect(stepped.slice(1).every((group) => group.boundaryReason === 'mean-change')).toBe(true)
  })
  ...
  test('does not bridge a record with a missing required population mean', () => {
    const groups = buildMethylationVisualGroups([
      point(1), point(2, Number.NaN), point(3),
    ])
    expect(groups.map((group) => group.siteCount)).toEqual([1, 1])
    expect(groups[1].boundaryReason).toBe('invalid-or-missing-value')
  })

  test('keeps 10,000 displayed points bounded by the configured site cap', () => {
