...
const sites: MethylationSummaryPoint[] = [
  { chrom: 'chr22', pos1: 100, pos2: 101, mean_methylation: 60, mean_coverage: 24, num_samples: 286, std_methylation: 5, min_methylation: 50, max_methylation: 70 },
  { chrom: 'chr22', pos1: 150, pos2: 151, mean_methylation: 65, mean_coverage: 3, num_samples: 8, std_methylation: 7, min_methylation: 45, max_methylation: 75 },
]
...
describe('Methylation context components', () => {
  ...
  test('shows group evidence, conservative interpretation, and constituent sites', () => {
    ...
    expect(text).toContain('does not establish functional effect, imprinting, pathogenicity, or diagnosis')
    expect(text).not.toMatch(/maternal|paternal/i)

    const showButton = tree.root.findAllByType('button').find((button) => renderedText(button.props.children).includes('Show constituent'))!
    ...
    const switchButton = tree.root.findAllByType('button').find((button) => renderedText(button.props.children).includes('Switch to CpG sites'))!
