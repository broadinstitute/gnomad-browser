...
const site = (mean_coverage: number, num_samples: number): MethylationSummaryPoint => ({
  chrom: 'chr22', pos1: 1, pos2: 2, mean_methylation: 50, mean_coverage, num_samples,
})
const copy = (meanDepth: number, representedSites = 10, totalSites = 10) => ({ meanDepth, representedSites, totalSites })
...
describe('methylation display support', () => {
  test('classifies adequate, depth-limited, sample-limited, and combined site support', () => {
    ...
    expect(classifyPopulationSupport(site(4, 4)).reasons.join(' ')).toContain('4 observed sample totals')
  })

  test('does not render a missing population mean as a zero-valued state', () => {
    expect(classifyPopulationSupport({ ...site(20, 100), mean_methylation: Number.NaN }).state).toBe('missing')
  })
  ...
  ;([
    ...
    [5, 'uneven'],
    [20, 'uneven'],
  ] as Array<[number, 'balanced-enough' | 'uneven']>).forEach(([ratio, expected]) => {
