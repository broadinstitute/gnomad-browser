...
We calculate the expected number of variants for all bases with median AN percent ≥ 20 in our exome samples using a mutational model that corrects for local sequence context and CpG methylation levels. Following the methods described in section 4.1 of the supplement in [Karczewski _et al._ Nature 2020](https://www.nature.com/articles/s41586-020-2308-7), we calculate a coverage model for sites with median AN percent between 20% and 90% in the gnomAD exome samples and use this model to adjust expected variant counts at low coverage sites.
