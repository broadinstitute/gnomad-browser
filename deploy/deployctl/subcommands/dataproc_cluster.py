...
def start_cluster(name: str, cluster_args: typing.List[str]) -> None:
    ...
    with open(os.path.join(DATA_PIPELINE_DIRECTORY, "requirements.txt")) as requirements_file:
        ...
        # requirements.txt is generated by uv (see data-pipeline/CLAUDE.md) and includes a header
        # comment; skip comments and blank lines so only package specifiers are passed to hailctl.
        requirements = [
            line.strip() for line in requirements_file.readlines() if line.strip() and not line.strip().startswith("#")
        ]
