...
const PositionAxis = ({ scalePosition, width }) => {
  ...
  const ticks = tickPositions.map(x => (
    <g key={x}>
      ...
      <text x={x} y={height - 7} textAnchor="center" style={{ fontSize: '10px' }}>
        {scalePosition.invert(x)}
      </text>
    </g>
  ))
  ...
  return (
    <svg height={height} width={width}>
      ...
      <line x1={0} y1={height - 7} x2={0} y2={height} stroke="black" strokeWidth={2} />
      <line x1={width} y1={height - 7} x2={width} y2={height} stroke="black" strokeWidth={2} />
      {ticks}
    </svg>
  )
}
