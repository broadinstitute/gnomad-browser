...
Tests and advanced use may set `LR_Y1_CLICKHOUSE_URL` directly and may explicitly override `LR_Y1_CLICKHOUSE_DATABASE`. No run IDs are supplied in the environment. At startup, the API reads `lr_y1_load_runs`, resolves exactly one terminal `accepted_frozen` canonical run for each present cohort, validates one consistent release/reference/chromosome/scope, and verifies canonical rows. One or both cohorts may be present. An absent cohort is an unavailable capability; zero usable cohorts, ambiguous accepted runs, loading/failed-only runs, malformed schema, missing canonical rows, missing HGSVC carriers, or any AoU carrier rows prevent startup.

The GraphQL `lr_cohort` argument defaults to `hgsvc_hprc`. Summary queries support whichever of `hgsvc_hprc` and `aou` startup discovered. AoU remains summary-only and never enters carrier or haplotype query paths. Every Y1 query and cache key uses the discovered run ID. ALT-expanded browser IDs use `<exact-source-id>~<alt-index>`, while `source_variant_id` preserves the source ID byte-for-byte.

Metadata and ancillary tables are optional capabilities in the same configured database. Missing or incomplete optional schemas are reported as unavailable, not as zero rows and not as startup failures. The ClickHouse client always sets `readonly=1`. Use `long_read_y1_provenance(lr_cohort:, chrom:)` for the generic accepted-Y1 identity and capability labels; accepted primary labels include database, cohort, run ID, scope, and terminal state.
