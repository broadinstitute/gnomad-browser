...
The human-facing LR identity UI adds nullable `source_variant_id`, `alt_index`, and `alt_count` fields to both `LongReadVariant` and the standard variant page's `LongReadVariantDetails`. Deploy the API schema before (or atomically with) the browser bundle that selects these fields; an older GraphQL API correctly rejects unknown selections. Because `start_lr_dev.sh` does not watch schema files, restart its API process after changing `.graphql` definitions before validating the variant page.
