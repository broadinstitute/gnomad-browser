...
Advanced use may set `LR_Y1_CLICKHOUSE_URL` and the identifier-validated `LR_Y1_CLICKHOUSE_DATABASE`. With no other routing configuration, startup preserves the current behavior: it resolves one terminal `accepted_frozen` run per present cohort and verifies its canonical rows. A presentation process may additionally provide `LR_Y1_RUN_MAP` as JSON mapping `cohort -> chromosome -> exact run ID`. Each configured run is routed only after startup proves its expected task receipts are complete and uniquely accepted, logical and physical rejects are zero, and receipt counts exactly match canonical rows. This path does not require primary finalization.
...
Optional presentation ancillaries use `LR_Y1_ANCILLARY_ROUTES`, mapping modality and cohort to an exact `{database, run_id}`. Coverage and STR may be configured independently for HGSVC/HPRC and AoU; sample-total methylation is HGSVC/HPRC-only. Startup validates each configured schema and nonempty exact route before enabling it. Unconfigured modalities remain unavailable rather than returning invented zeroes. The ClickHouse client always sets `readonly=1`. Use `long_read_y1_provenance(lr_cohort:, chrom:)` for the generic accepted-Y1 identity and capability labels; accepted primary labels include database, cohort, run ID, scope, and terminal state.
