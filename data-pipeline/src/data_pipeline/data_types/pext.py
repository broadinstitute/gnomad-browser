...
def prepare_base_level_pext(base_level_pext_path):
    ...
    subprocess.run(
        ["hdfs", "dfs", "-cp", "file://" + os.path.join(tmp_dir, "regions.tsv"), os.path.join("/tmp/regions.tsv")],
        check=True,
    )
    ...
    ds = hl.import_table("/tmp/regions.tsv", min_partitions=100, missing="", types=types)
