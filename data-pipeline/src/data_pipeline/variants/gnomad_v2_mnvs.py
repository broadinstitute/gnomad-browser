...
def replace_quote_char(path):
    ...
    output_path = os.path.basename(path) + ".corrected.tsv"
    with hl.hadoop_open(path, "r") as f_in:
        with hl.hadoop_open(output_path, "w") as f_out:
        # with tempfile.NamedTemporaryFile(mode="w", suffix=".tsv", delete=False) as f_out:
            ...
            for row in reader:
                writer.writerow(row)
            # return "file://" + f_out.name
    return output_path


def prepare_gnomad_v2_mnvs(mnvs_path, three_bp_mnvs_path):
    mnvs_path = replace_quote_char("file://" + mnvs_path)
    mnvs = import_mnv_file(mnvs_path, quote="'")

    three_bp_mnvs_path = replace_quote_char("file://" + three_bp_mnvs_path)
