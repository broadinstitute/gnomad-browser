...
def collect_structural_variants_by_gene(structural_variants_path):
    ds = hl.read_table(structural_variants_path)
    ds = ds.explode(ds.genes, name="gene_symbol")
    ds = ds.group_by(ds.gene_symbol).aggregate(variants=hl.agg.collect(ds.row))

    # Set major consequence in the selected gene
    ds = ds.annotate(
        variants=ds.variants.map(
            lambda variant: variant.annotate(
                major_consequence=hl.rbind(
                    variant.consequences.find(lambda csq: csq.genes.contains(ds.gene_symbol)),
                    lambda csq: hl.or_else(csq.consequence, hl.or_missing(variant.intergenic, "intergenic")),
                )
            )
        )
    )
