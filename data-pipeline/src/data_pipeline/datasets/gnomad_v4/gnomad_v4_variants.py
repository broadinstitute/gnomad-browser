...
def prepare_gnomad_v4_variants_helper(input_path: str, exomes_or_genomes: str):
    ...
    # TK remove filter
    AMY1B_CHROM = hl.literal("chr1")
    AMY1B_START = hl.literal(103687415)
    AMY1B_STOP = hl.literal(103696454)
    RGPD5_CHROM = hl.literal("chr2")
    RGPD5_START = hl.literal(109792758)
    RGPD5_STOP = hl.literal(109857705)
    MAGEA9_CHROM = hl.literal("chrX")
    MAGEA9_START = hl.literal(149781930)
    MAGEA9_STOP = hl.literal(149787737)
    CHD7_CHROM = hl.literal("chr8")
    CHD7_START = hl.literal(60678740)
    CHD7_STOP = hl.literal(60868028)
    CBS_CHROM = hl.literal("chr21")
    CBS_START = hl.literal(43053191)
    CBS_STOP = hl.literal(43076943)
    ABCG8_CHROM = hl.literal("chr2")
    ABCG8_START = hl.literal(43831942)
    ABCG8_STOP = hl.literal(43882988)
    ARHGEF5_CHROM = hl.literal("chr7")
    ARHGEF5_START = hl.literal(144355288)
    ARHGEF5_STOP = hl.literal(144380632)
    ACKR4_CHROM = hl.literal("chr3")
    ACKR4_START = hl.literal(132597270)
    ACKR4_STOP = hl.literal(132618967)
    PCSK9_CHROM = hl.literal("chr1")
    PCSK9_START = hl.literal(55039447)
    PCSK9_STOP = hl.literal(55064852)
    TTN_CHROM = hl.literal("chr2")
    TTN_START = hl.literal(178525989)
    TTN_STOP = hl.literal(178830802)
    RNU4ATAC_CHROM = hl.literal("chr2")
    RNU4ATAC_START = hl.literal(121530881)
    RNU4ATAC_STOP = hl.literal(121531007)
    ds = ds.filter(
        ((ds.locus.contig == AMY1B_CHROM) & (ds.locus.position >= AMY1B_START) & (ds.locus.position <= AMY1B_STOP))
        | ((ds.locus.contig == RGPD5_CHROM) & (ds.locus.position >= RGPD5_START) & (ds.locus.position <= RGPD5_STOP))
        | ((ds.locus.contig == MAGEA9_CHROM) & (ds.locus.position >= MAGEA9_START) & (ds.locus.position <= MAGEA9_STOP))
        | ((ds.locus.contig == CHD7_CHROM) & (ds.locus.position >= CHD7_START) & (ds.locus.position <= CHD7_STOP))
        | ((ds.locus.contig == CBS_CHROM) & (ds.locus.position >= CBS_START) & (ds.locus.position <= CBS_STOP))
        | ((ds.locus.contig == ABCG8_CHROM) & (ds.locus.position >= ABCG8_START) & (ds.locus.position <= ABCG8_STOP))
        | (
            (ds.locus.contig == ARHGEF5_CHROM)
            & (ds.locus.position >= ARHGEF5_START)
            & (ds.locus.position <= ARHGEF5_STOP)
        )
        | ((ds.locus.contig == ACKR4_CHROM) & (ds.locus.position >= ACKR4_START) & (ds.locus.position <= ACKR4_STOP))
        | ((ds.locus.contig == PCSK9_CHROM) & (ds.locus.position >= PCSK9_START) & (ds.locus.position <= PCSK9_STOP))
        | ((ds.locus.contig == TTN_CHROM) & (ds.locus.position >= TTN_START) & (ds.locus.position <= TTN_STOP))
        | (
            (ds.locus.contig == RNU4ATAC_CHROM)
            & (ds.locus.position >= RNU4ATAC_START)
            & (ds.locus.position <= RNU4ATAC_STOP)
        )
    )
