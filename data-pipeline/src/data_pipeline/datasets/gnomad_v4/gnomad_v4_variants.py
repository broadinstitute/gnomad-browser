...
variants_by_locus = variants_by_locus.group_by("locus").aggregate(variants=hl.agg.collect(variants_by_locus.row_value))
                ...
                or "all": hl.struct(
                    ...
                    populations=[
                        hl.struct(
                            ...
                            homozygote_count=hl.or_else(freq(ds, subset=subset, pop=pop, sex=sex).homozygote_count, 0),
                        )
...
ds = ds.annotate(
    genome=ds.genome.annotate(
        age_distribution=hl.struct(het=ds.histograms.age_hists.age_hist_ht, hom=ds.histograms.age_hists.age_hist_hom)
    )
)
